export const media = {
  largeScreen: '@media (min-width: 1280px)',
  uptolargeScreen: '@media (max-width: 1279px)',
  modalView: '@media (min-width: 832px) and (max-width: 1279px)',
  mediumScreen: '@media (min-width: 1024px)',
  portrait: '@media screen and (orientation: portrait)',
  landscape: '@media screen and (orientation: landscape)',
  mobileScreen: '@media only screen and (max-width: 831px)',
  realMobile: '@media only screen and (max-width: 767px)',
  webView: '@media only screen and (min-width: 832px)',
  smallScreen: '@media (max-width: 376px)',
  bigMobileScreen: '@media (max-width: 425px)',
  tabletMobile: '@media only screen and (max-width: 1279px)',

};
