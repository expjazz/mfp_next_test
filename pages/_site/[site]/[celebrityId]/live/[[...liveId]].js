...
function PurchaseLiveCall(props) {
	...
	const getTitle = () => router.query?.liveId && correctQueryTitle(router.query.liveId) !== 'index' ? correctQueryTitle(router.query.liveId) : null;
	...
	const productClick = funStuff => () => {
		...
		router.push(`/${userId}/live/${funStuff.slug}`, undefined, { shallow: true });
	};
	...
	if (!isEmpty(selected) && title) {
		return (
			<React.Fragment>
				...
				<SeoHeader
					title={t('purchase_flow.live_call.meta_title', {
						starNM,
						title: selected?.title,
						duration: selected?.meeting_duration,
					})}
					shareImage={
						selected?.sample_image ? selected?.sample_image
							: getAvtar(celebrityData?.user.avatar_photo)
					}
					description={t('purchase_flow.live_call.meta_desc', {
						starNM,
						duration: selected?.meeting_duration,
					})}
					keywords={t('purchase_flow.live_call.meta_keywords', {
						starNM,
	...
	return (
		<>
			<Wrapper reqDisplay>
				<SeoHeader
					title={t('purchase_flow.live_call.meta_title1', {
						purchaser: partnerData.purchaser_plural_name,
					})}
					shareImage={
						selected?.sample_image ? selected?.sample_image
							: getAvtar(celebrityData?.user.avatar_photo)
					}
					description={t('purchase_flow.live_call.meta_desc1', {
						starNM
					})}
					keywords={t('purchase_flow.live_call.meta_keywords', {
						starNM,
				...
				/>
				<HeadingH1>{t('purchase_flow.live_calls')}</HeadingH1>
...
PurchaseLiveCall.getLayout = page => (
...
export default PurchaseLiveCall;
