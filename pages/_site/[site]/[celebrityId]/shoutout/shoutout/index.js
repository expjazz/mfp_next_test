...
import { useFetchOccasions, useGetCelebrityData } from 'customHooks/reactQueryHooks';
...
function ShoutoutProduct(props) {
	...
	const { showContent, updateLocalStore, toggContent } = useContext(
		StarContext,
	);
	...
	const onSubmit = ({ lang, promoCode, discount }) => {
		const payload = {
			...
			from_audio_file_external_uploader: bookingData?.from_external_uploader,
			to_audio_file_external_uploader: bookingData?.to_external_uploader || bookingData?.for_external_uploader,
			host_audio_file_external_uploader: bookingData?.host_external_uploader,
			honor_audio_file_external_uploader: bookingData?.honor_external_uploader,
			from_audio_file: bookingData?.from || getAudioFile('from'),
			to_audio_file: bookingData?.for || getAudioFile('for'),
			host_audio_file: bookingData?.host || getAudioFile('host'),
			honor_audio_file: bookingData?.honor || getAudioFile('honor'),
	...
	const zeroPayment = bookingID => {
		if (paymentDetails.type === 'booking') {
			zeroPayBooking(
				{
					amount: paymentDetails.rate,
					reference: bookingID,
					promoCode: paymentDetails.promoCode.code,
				},
				dispatch,
	...
	useEffect(() => {
		if (window.dataLayer) {
			window.dataLayer.push({
				...
				product_type: 'shoutout',
				product_id:  generateProductId('shoutout', {
	...
	const saveAudioRecording = () => {
		console.log('saveAudioRecording');
	};
	return (
		<>
...
ShoutoutProduct.getLayout = page => (
...
export async function getStaticProps(props) {
	...
	try {
		...
		await queryClient.prefetchQuery(['shoutout-occasions', entityId], () => fetchOccasionlist(1, entityId));
...
export default ShoutoutProduct;
