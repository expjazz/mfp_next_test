...
const VodacomProfilePhoto = props => {
	...
	const uploadPicture = file => {
		...
		if (allowedExtensions.exec(file.target.value)) {
			...
			setFile({ croppedFile: file.target.files[0] });
		}
	};

	return (
		<Container className={`${props.classes.root} set-wrap`}>
			<Wrap imageUrl={imageData.imageUrl || props.profImg}>
				<Heading
					className="sub-head"
				...
				<section className="content-wrapper">
					...
					<Button
						disabled={!imageData.imageUrl}
						isDisabled={!imageData.imageUrl}
						onClick={() => props.updateProfilePhoto(file)}
					>
						{t('common.save')}
					</Button>
				</section>
...
VodacomProfilePhoto.defaultProps = {
	webHead: '',
	mobHead: '',
	profImg: '',
	classes: {},
...
export default VodacomProfilePhoto;
