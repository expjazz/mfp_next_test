...
const FunItem = props => {
  ...
  const inputChange = state => event => {
    ...
    if (state === 'price' || state === 'quantity') {
      ...
      if (/^\d*\.?\d*$/.test(value) && value < 9999999) {
        ...
        props.confirmSave({ saved: false, route: 'fun-stuff' });
        return;
      }
      return;
    }
  ...
  const validateForm = () => {
    const validate = [
      ...
      !isEmpty(formData.required_info),
      !isEmpty(formData.delivery_method),
  ...
  const addNew = fileName => {
    const payload = {
      ...formData,
      delivery_method: formData.delivery_method.id,
      allow_fan_image: allowOptions.image,
      meeting_duration: formData.meeting_duration.id,
      allow_fan_url: allowOptions.link,
  ...
  return ready && (
    <React.Fragment>
      ...
      <Layout hasImage={isEmpty(baseUrls)} className="content-wrapper">
        <BackHeader backHandler={backHandler} label={t('services.funstuff.heading')} noHelp />
        ...
        >
          <InputContainer className="input-container">
            <Dropdown
              ...
              selected={formData.delivery_method || {}}
              options={props.deliveryTypes.filter(
                del => del.delivery_type !== deliveryMethods.videoCalls,
              )}
              labelKey='title'
              valueKey="id"
              onChange={dropChange('delivery_method')}
              placeHolder={t('services.funstuff.deliveryPlaceholder')}
              secondary
            ...
            <CInput
              ...
              inputCls="CInput"
              label={t('services.funstuff.titlePlaceholder')}
            ...
            <CInput
              containerCls="description-wrp"
              inputCls="CInput"
              label={t('services.funstuff.descriptionLabel')}
            ...
            <div className="text-area-wrp">
              <span className="info-head">
                {
                  t('services.funstuff.info', {
              ...
              <TextArea
                ...
                inputProps={{
                  ...
                  maxLength: 255,
                  placeholder: t('services.funstuff.infoPlaceholder'),
                  className: 'textarea',
                }}
              />
            </div>
